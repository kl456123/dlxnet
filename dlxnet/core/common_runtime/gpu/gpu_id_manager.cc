...
namespace dlxnet{
    namespace {
        ...
        class TfToPlatformGpuIdMap {
            public:
                ...
                Status Insert(TfGpuId tf_gpu_id, PlatformGpuId platform_gpu_id)
                    LOCKS_EXCLUDED(mu_){
                        ...
                        {
                            mutex_lock lock(mu_);
                            result = id_map_.insert({tf_gpu_id, platform_gpu_id});
                        }
                        if (!result.second && platform_gpu_id != result.first->second) {
                            return errors::AlreadyExists(
                                    "TensorFlow device (GPU:", tf_gpu_id,
                                    ") is being mapped to "
                                    "multiple CUDA devices (",
                                    platform_gpu_id, " now, and ", result.first->second,
                ...
                bool Find(TfGpuId tf_gpu_id, PlatformGpuId* platform_gpu_id) const
                    LOCKS_EXCLUDED(mu_) {
                        ...
                        auto result = id_map_.find(tf_gpu_id);
            ...
            private:
                ...
                friend class ::dlxnet::GpuIdManager;
    ...
    Status GpuIdManager::TfToPlatformGpuId(TfGpuId tf_gpu_id,
        ...
        return errors::NotFound("TensorFlow device GPU:", tf_gpu_id,
                " was not registered");
    }
}// namespace dlxnet
