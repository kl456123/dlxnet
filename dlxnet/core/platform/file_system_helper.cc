...
#include "dlxnet/core/platform/file_system_helper.h"
...
#include "dlxnet/core/platform/threadpool.h"
...
Status GetMatchingPaths(FileSystem* fs, Env* env, const string& pattern,
  ...
  while (!dir_q.empty()) {
    ...
    if (s.code() == dlxnet::error::PERMISSION_DENIED) {
      continue;
    }
    ret.Update(s);
            ...
            [fs, &current_dir, &children, &fixed_prefix,
             &children_dir_status](int i) {
              ...
              if (!absl::StartsWith(child_path, fixed_prefix)) {
                children_dir_status[i] = Status(dlxnet::error::CANCELLED,
    ...
    for (int i = 0; i < children.size(); ++i) {
      ...
      if (children_dir_status[i].code() == dlxnet::error::CANCELLED) {
        continue;
      }
