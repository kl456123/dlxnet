...
#ifndef DLXNET_CORE_PLATFORM_ERRORS_H_
#define DLXNET_CORE_PLATFORM_ERRORS_H_
...
#include "dlxnet/core/platform/str_util.h"
#include "dlxnet/core/platform/strcat.h"
...
void AppendToMessage(::dlxnet::Status* status, Args... args) {
  *status = ::dlxnet::Status(
      status->code(),
      ::dlxnet::strings::StrCat(status->error_message(), "\n\t", args...));
}

// For propagating errors when calling a function.
#define TF_RETURN_IF_ERROR(...)                          \
  do {                                                   \
    ::dlxnet::Status _status = (__VA_ARGS__);        \
    if (TF_PREDICT_FALSE(!_status.ok())) return _status; \
  } while (0)

#define TF_RETURN_WITH_CONTEXT_IF_ERROR(expr, ...)                  \
  do {                                                              \
    ::dlxnet::Status _status = (expr);                          \
    if (TF_PREDICT_FALSE(!_status.ok())) {                          \
      ::dlxnet::errors::AppendToMessage(&_status, __VA_ARGS__); \
      return _status;                                               \
    }                                                               \
  } while (0)
...
#define DECLARE_ERROR(FUNC, CONST)                                       \
  template <typename... Args>                                            \
  ::dlxnet::Status FUNC(Args... args) {                              \
    return ::dlxnet::Status(                                         \
        ::dlxnet::error::CONST,                                      \
        ::dlxnet::strings::StrCat(                                   \
            ::dlxnet::errors::internal::PrepareForStrCat(args)...)); \
  }                                                                      \
  inline bool Is##FUNC(const ::dlxnet::Status& status) {             \
...
string FormatNodeNamesForError(const T& names) {
  return absl::StrJoin(names, ", ", [](string* output, const string& s) {
    ::dlxnet::strings::StrAppend(output, FormatNodeNameForError(s));
  });
}
// LINT.IfChange
...
string FormatColocationNodeForError(const T& names) {
  return absl::StrJoin(names, ", ", [](string* output, const string& s) {
    ::dlxnet::strings::StrAppend(output, FormatColocationNodeForError(s));
  });
}
...
using ::dlxnet::error::OK;

}  // namespace errors
}  // namespace dlxnet

#endif  // DLXNET_CORE_PLATFORM_ERRORS_H_
