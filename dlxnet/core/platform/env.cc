...
#undef DeleteFile
#undef CopyFile
#include "dlxnet/core/platform/windows/wide_char.h"
...
namespace {
class FileStream : public ::dlxnet::protobuf::io::ZeroCopyInputStream {
 public:
...
Status WriteBinaryProto(Env* env, const string& fname,
                        const ::dlxnet::protobuf::MessageLite& proto) {
  string serialized;
...
Status ReadBinaryProto(Env* env, const string& fname,
                       ::dlxnet::protobuf::MessageLite* proto) {
  std::unique_ptr<RandomAccessFile> file;
  ...
  ::dlxnet::protobuf::io::CodedInputStream coded_stream(stream.get());
...
Status WriteTextProto(Env* env, const string& fname,
                      const ::dlxnet::protobuf::Message& proto) {
#if !defined(TENSORFLOW_LITE_PROTOS)
  string serialized;
  if (!::dlxnet::protobuf::TextFormat::PrintToString(proto, &serialized)) {
...
Status ReadTextProto(Env* env, const string& fname,
                     ::dlxnet::protobuf::Message* proto) {
#if !defined(TENSORFLOW_LITE_PROTOS)
  std::unique_ptr<RandomAccessFile> file;
  TF_RETURN_IF_ERROR(env->NewRandomAccessFile(fname, &file));
  std::unique_ptr<FileStream> stream(new FileStream(file.get()));

  if (!::dlxnet::protobuf::TextFormat::Parse(stream.get(), proto)) {
    TF_RETURN_IF_ERROR(stream->status());
