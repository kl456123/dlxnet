...
#ifndef DLXNET_CORE_PLATFORM_FILE_SYSTEM_H_
#define DLXNET_CORE_PLATFORM_FILE_SYSTEM_H_
...
#include "dlxnet/core/platform/cord.h"
#include "dlxnet/core/platform/errors.h"
#include "dlxnet/core/platform/file_statistics.h"
...
class FileSystem {
 public:
  ...
  virtual dlxnet::Status NewRandomAccessFile(
  ...
  /// and the object should be deleted when is not used.
  virtual dlxnet::Status NewWritableFile(
  ...
  /// The ownership of the returned WritableFile is passed to the caller
  /// and the object should be deleted when is not used.
  virtual dlxnet::Status NewAppendableFile(
      const string& fname, std::unique_ptr<WritableFile>* result) = 0;
  ...
  /// and the object should be deleted when is not used.
  virtual dlxnet::Status NewReadOnlyMemoryRegionFromFile(
      const string& fname, std::unique_ptr<ReadOnlyMemoryRegion>* result) = 0;

  /// Returns OK if the named path exists and NOT_FOUND otherwise.
  virtual dlxnet::Status FileExists(const string& fname) = 0;
  ...
  virtual dlxnet::Status GetChildren(const string& dir,
  ...
  ///                    implemented
  virtual dlxnet::Status GetMatchingPaths(const string& pattern,
                                              std::vector<string>* results) = 0;

  /// \brief Obtains statistics for the given path.
  virtual dlxnet::Status Stat(const string& fname,
                                  FileStatistics* stat) = 0;

  /// \brief Deletes the named file.
  virtual dlxnet::Status DeleteFile(const string& fname) = 0;
  ...
  virtual dlxnet::Status CreateDir(const string& dirname) = 0;
  ...
  virtual dlxnet::Status RecursivelyCreateDir(const string& dirname);

  /// \brief Deletes the specified directory.
  virtual dlxnet::Status DeleteDir(const string& dirname) = 0;
  ...
  ///                    implemented
  virtual dlxnet::Status DeleteRecursively(const string& dirname,
                                               int64* undeleted_files,
                                               int64* undeleted_dirs);

  /// \brief Stores the size of `fname` in `*file_size`.
  virtual dlxnet::Status GetFileSize(const string& fname,
                                         uint64* file_size) = 0;

  /// \brief Overwrites the target if it exists.
  virtual dlxnet::Status RenameFile(const string& src,
                                        const string& target) = 0;

  /// \brief Copy the src to target.
  virtual dlxnet::Status CopyFile(const string& src, const string& target);
  ...
  virtual dlxnet::Status IsDirectory(const string& fname);
...
class RandomAccessFile {
 public:
  ...
  /// filesystem.
  virtual dlxnet::Status Name(StringPiece* result) const {
  ...
  virtual dlxnet::Status Read(uint64 offset, size_t n, StringPiece* result,
                                  char* scratch) const = 0;

  // TODO(ebrevdo): Remove this ifdef when absl is updated.
#if defined(PLATFORM_GOOGLE)
  /// \brief Read up to `n` bytes from the file starting at `offset`.
  virtual dlxnet::Status Read(uint64 offset, size_t n,
...
class WritableFile {
 public:
  ...
  virtual dlxnet::Status Append(StringPiece data) = 0;

  // TODO(ebrevdo): Remove this ifdef when absl is updated.
#if defined(PLATFORM_GOOGLE)
  // \brief Append 'data' to the file.
  virtual dlxnet::Status Append(const absl::Cord& cord) {
  ...
  virtual dlxnet::Status Tell(int64* position) {
    *position = -1;
...
class FileSystemRegistry {
 public:
  ...
  virtual dlxnet::Status Register(const std::string& scheme,
                                      Factory factory) = 0;
  virtual dlxnet::Status Register(
      const std::string& scheme, std::unique_ptr<FileSystem> filesystem) = 0;
  virtual FileSystem* Lookup(const std::string& scheme) = 0;
  virtual dlxnet::Status GetRegisteredFileSystemSchemes(
      std::vector<std::string>* schemes) = 0;
};

}  // namespace dlxnet

#endif  // DLXNET_CORE_PLATFORM_FILE_SYSTEM_H_
