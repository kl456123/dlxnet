...
Status FileSystem::IsDirectory(const string& name) {
  ...
  return Status(dlxnet::error::FAILED_PRECONDITION, "Not a directory");
}
...
Status FileSystem::RecursivelyCreateDir(const string& dirname) {
  ...
  while (!remaining_dir.empty()) {
    ...
    if (exists_status.ok()) {
      ...
      } else if (directory_status.code() == dlxnet::error::UNIMPLEMENTED) {
  ...
  for (const StringPiece sub_dir : sub_dirs) {
    ...
    if (!status.ok() && status.code() != dlxnet::error::ALREADY_EXISTS) {
      return status;
    }
  }
  return Status::OK();
