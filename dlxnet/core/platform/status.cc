...
#include "dlxnet/core/platform/stacktrace.h"
...
Status::Status(dlxnet::error::Code code, StringPiece msg) {
...
string* TfCheckOpHelperOutOfLine(const ::dlxnet::Status& v,
                                 const char* msg) {
...
Status StatusGroup::as_summary_status() const {
  ...
  if (!nonderived_statuses.empty()) {
    ...
    int index = 0;
    auto code = dlxnet::error::CANCELLED;
    for (auto& s : nonderived_statuses) {
      ...
      if (code == dlxnet::error::CANCELLED &&
          s.code() != dlxnet::error::CANCELLED) {
        code = s.code();
      }
