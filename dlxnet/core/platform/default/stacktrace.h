...
#ifndef DLXNET_CORE_PLATFORM_DEFAULT_STACKTRACE_H_
#define DLXNET_CORE_PLATFORM_DEFAULT_STACKTRACE_H_
...
#include <sstream>
#include <string>
#include "dlxnet/core/platform/abi.h"
...
inline std::string CurrentStackTrace() {
  ...
  for (int i = 0; i < buffer_size; ++i) {
    ...
    std::string demangled = dlxnet::port::MaybeAbiDemangle(symbol);
    if (demangled.length()) {
...
#endif  // DLXNET_CORE_PLATFORM_DEFAULT_STACKTRACE_H_
