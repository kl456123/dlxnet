...
#include "dlxnet/core/platform/default/posix_file_system.h"
#include "dlxnet/core/platform/env.h"
#include "dlxnet/core/platform/load_library.h"
...
mutex name_mutex(dlxnet::LINKER_INITIALIZED);
...
class PosixEnv : public Env {
 public:
  ...
  Status LoadLibrary(const char* library_filename, void** handle) override {
    return dlxnet::internal::LoadLibrary(library_filename, handle);
  }

  Status GetSymbolFromLibrary(void* handle, const char* symbol_name,
                              void** symbol) override {
    return dlxnet::internal::GetSymbolFromLibrary(handle, symbol_name,
                                                      symbol);
  }

  string FormatLibraryFileName(const string& name,
                               const string& version) override {
    return dlxnet::internal::FormatLibraryFileName(name, version);
  }

  string GetRunfilesDir() override {
    string bin_path = this->GetExecutablePath();
    string runfiles_suffix = ".runfiles/org_dlxnet";
