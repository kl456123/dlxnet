...
// in dlxnet::mutex is big enough.
static_assert(sizeof(nsync::nsync_mu) <= sizeof(internal::MuData),
              "dlxnet::internal::MuData needs to be bigger");
...
// need to be included in every file that uses dlxnet's mutex.
...
// condition variable in dlxnet::condition_variable is big enough.
static_assert(sizeof(nsync::nsync_cv) <= sizeof(internal::CVData),
              "dlxnet::internal::CVData needs to be bigger");
...
// do not need to be included in every file that uses dlxnet's
// condition_variable.
